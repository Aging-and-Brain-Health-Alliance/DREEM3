...
[N,Ch]=size(data); %CH =Channels; N= samples
Nwin=round(Fs*movingwin(1)); % number of samples in window =6k
Nstep=round(movingwin(2)*Fs); % number of samples to step through =1k
nfft=max(2^(nextpow2(Nwin)+pad),Nwin); % here it's 2^14 = padded up value of number of samples to be subjected to fft;
f=getfgrid(Fs,nfft,fpass); Nf=length(f); %Nf =1618 or 19
