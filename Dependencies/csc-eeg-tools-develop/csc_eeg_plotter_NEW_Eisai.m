function EEG = csc_eeg_plotter_Eisai(varargin)
...
handles.filter_options = [0.7 35; 10 35; 0.3 10; 0.1 10]; % default filter bands
%handles.filter_options = [NaN]; % default filter bands
handles.epoch_length = 30; % default viewing window
handles.n_disp_chans = 7; % number of initial channels to display
...
handles.negative_up = true; % negative up by default (for clinicians)
handles.number_of_event_types = 7; % how many event types do you want
...
% ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
EEG_chanlabels = {EEG.chanlocs.labels};
...
if flag_replot
    ...
    for n = 1 : handles.n_disp_chans
        ...
        handles.labels(n) = ...
            text(0.25, toAdd(n, 1) + scale / 5, EEG_chanlabels{chn},...
