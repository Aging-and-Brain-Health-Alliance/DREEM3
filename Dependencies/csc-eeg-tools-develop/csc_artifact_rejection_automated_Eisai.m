...
switch method
    ...
    case 'wispic'
        ...
        % process the optional attributes (like epoch_length) //AD
...
%         % calculate the bad epochs in time
         bad_epochs_time = (find(sum(sum(EEG.bad_epochs,3)))* options.epoch_length - ...
                                                options.epoch_length);
%         
...
varargin = varargin{1};  % in this case it would be 'EPOCH_LENGTH' //AD

options = struct(...
    'ave_ref', 0, ...
    'bad_channels', [], ........
    'bands_of_interest', [1, 5], ... % as in delta, theta, alpha, beta1-beta 2? //AD
    'default_percentile', 99, ... %originally at 99 //KL 7/16/2020 99.8
    'method', '',...%'semi_automatic', ... %change this back to semi-automatic if you want to use the gui //KL 7/16/2020
    'save_file',  0, ...
    'save_name', saveName, ...
    'epoch_length', 6,...
    'freq_limit', 40 ); %
